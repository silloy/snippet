...
def clipboard_is_ok(text):
    ...
    # clipboard.paste() is a listener function
...
def finish_copy():
    global break_flag
    s = raw_input("Press Enter to begin translate!\n")
    if s == '':
        break_flag = False
...
clipboard.copy('')  # first clear the clipboard
previous_text = u''
break_flag = True
while break_flag:
    if clipboard_is_ok(previous_text):
        previous_text = clipboard.paste()  # save the last time's clipboard
        original_text = clipboard.paste().encode('utf-8')  # clipboard.paste() is an unicode object
